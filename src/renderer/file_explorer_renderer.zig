...
pub fn render(window_width: f32, window_height: f32, titlebar_h: f32) void {
    ...
    const header_y = window_height - titlebar_h - file_explorer.HEADER_HEIGHT;
    ...
    const label_end = titlebar.renderTextLimited(mode_label, panel_x + 12, header_y + (file_explorer.HEADER_HEIGHT - font.g_titlebar_cell_height) / 2, mode_color, explorer_w - 24);
    _ = titlebar.renderTextLimited(" Explorer", label_end, header_y + (file_explorer.HEADER_HEIGHT - font.g_titlebar_cell_height) / 2, header_text, explorer_w - (label_end - panel_x) - 12);
    gl_init.renderQuad(panel_x, header_y, explorer_w, 1, border_color);

    // File entries
    const list_top_px = titlebar_h + file_explorer.HEADER_HEIGHT;
    ...
    while (i < file_explorer.g_entry_count) : (i += 1) {
        const row_y_from_top = @as(f32, @floatFromInt(i)) * file_explorer.ROW_HEIGHT - scroll;
        if (row_y_from_top + file_explorer.ROW_HEIGHT < 0) continue;
        if (row_y_from_top >= visible_height) break;

        const row_top_px = list_top_px + row_y_from_top;
        const row_y = window_height - row_top_px - file_explorer.ROW_HEIGHT;
        ...
        const row_hovered = blk: {
            ...
            const mx: f32 = @floatFromInt(win.mouse_x);
            const my: f32 = @floatFromInt(win.mouse_y);
            break :blk mx >= panel_x and mx < panel_x + explorer_w and my >= row_top_px and my < row_top_px + file_explorer.ROW_HEIGHT;
        };

        const is_selected = if (file_explorer.g_selected) |sel| sel == i else false;

        if (is_selected) {
            gl_init.renderQuad(panel_x, row_y, explorer_w, file_explorer.ROW_HEIGHT, selected_bg);
        } else if (row_hovered) {
            gl_init.renderQuad(panel_x, row_y, explorer_w, file_explorer.ROW_HEIGHT, hover_bg);
        }

        // Expand/collapse indicator for directories
        const text_x = panel_x + 8 + indent;
        const text_y = row_y + (file_explorer.ROW_HEIGHT - font.g_titlebar_cell_height) / 2;
    ...
    if (file_explorer.g_op_mode == .new_file or file_explorer.g_op_mode == .new_dir) {
        const new_row_top = list_top_px + @as(f32, @floatFromInt(file_explorer.g_entry_count)) * file_explorer.ROW_HEIGHT - scroll;
        if (new_row_top >= 0 and new_row_top < visible_height) {
            const new_row_y = window_height - new_row_top - file_explorer.ROW_HEIGHT;
            gl_init.renderQuad(panel_x, new_row_y, explorer_w, file_explorer.ROW_HEIGHT, selected_bg);
            const label = if (file_explorer.g_op_mode == .new_dir) "New folder: " else "New file: ";
            const op_label_end = titlebar.renderTextLimited(label, panel_x + 8, new_row_y + (file_explorer.ROW_HEIGHT - font.g_titlebar_cell_height) / 2, header_text, explorer_w - 16);
            renderInputField(op_label_end + 2, new_row_y + (file_explorer.ROW_HEIGHT - font.g_titlebar_cell_height) / 2, explorer_w - (op_label_end - panel_x) - 10, fg, accent);
        }
    } else if (file_explorer.g_op_mode == .confirm_delete) {
        const del_row_top = list_top_px + @as(f32, @floatFromInt(file_explorer.g_entry_count)) * file_explorer.ROW_HEIGHT - scroll;
        if (del_row_top >= 0 and del_row_top < visible_height) {
            const del_row_y = window_height - del_row_top - file_explorer.ROW_HEIGHT;
            const warn_bg = blend(bg, .{ 0.8, 0.2, 0.2 }, 0.2);
            gl_init.renderQuad(panel_x, del_row_y, explorer_w, file_explorer.ROW_HEIGHT, warn_bg);
            _ = titlebar.renderTextLimited("Delete? Enter=yes Esc=no", panel_x + 8, del_row_y + (file_explorer.ROW_HEIGHT - font.g_titlebar_cell_height) / 2, fg, explorer_w - 16);
        }
    }

    // Transfer status bar at bottom of panel (auto-hides after 5 seconds)
    if (file_explorer.g_transfer_status != .idle) {
